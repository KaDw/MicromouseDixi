...
class Simulator:
    ...
    def __init__(self, inputs, outputs, add_random_struct=False):
    ...
    def _update_module(self, i):
        ...
        if len(self.nodes) > self.modInNodeNum[i]:
            self.modules[i].input = self.nodes[self.modInNodeNum[i]]  # read input from node to module
            dt = self.deltaTime
            m = self.modules[i]
            m.update(dt)
