...
class RPN:
    ...
    default_value_of_subequations = 0
        ...
        for val in self.expression:
                if isinstance(val, float):
                    ...
                    if val == '-':
                        ...
                        result = op2 / op1 if op1 != 0.0 else 0
                    elif val == '**':
                        result = np.real(op2 ** op1 if op2 != 0.0 and op1 < 1 else 0)
                    elif val == 'log':
                        result = math.log(abs(op2), abs(op1)) if op1 != 0.0 and op2 != 0.0 else 0.0
                    else:
                        raise Exception("operand {} isn't fully supported".format(val))
                    stack.append(result)
...
class SimulatorRPN:
    ...
    def calc_dxdt(x, t, u, sim):
        var = [*u, *x]
        return np.real([e(var) for e in sim.state_equations])
    ...
    def mutate_rpn(rpn):
        ...
        if random.random() < 0.15:  # szansa na mutacje parametru/operacji
            e = rpn.expression
            ...
            if isinstance(e[n], float) or e[n][0] == 'p':
                ...
                if r < 0.33:  # zastapienie
                    ...
                    e[n] = math.e*random.random()*val
                else:  # dodawanie
                    e[n] = 10*(random.random()-0.5) + val
            elif e[n][0] == 'x':
                e[n] = rpn.random_param()
    ...
    def mutate(self):
        ...
        if random.random() < 0.005:  # dodanie nowej zmiennej stanu
            len_var += 1
